...
class Carriers:
   # 初始化
   def __init__(self, deviceID, wight, height):
      self.deviceID = deviceID
      self.resolution = (wight, height)
   ...
   def watch(self, imgPath, duration ,intervalTime, threshold = 0.7):
   ...
   # 监听图片出现 并且 点击
   def watchAndTouche(self, imgPath, duration ,intervalTime, threshold = 0.7):
   ...
   def watchNotSlide(self, imgPath, x1, y1, x2, y2, time, maxnumber = 20, threshold = 0.7):
      return watchNotSlide(self.deviceID, imgPath, x1, y1, x2, y2, time, maxnumber, threshold)

   # 监听图片出现 未出现 滑动 出现 点击
   def watchNotSlideAndTouche(self, imgPath, x1, y1, x2, y2, time, maxnumber = 20, threshold = 0.7):
      watchNotSlideAndClient(self.deviceID, imgPath, x1, y1, x2, y2, time, maxnumber, threshold)

   # 监听多张图片是否出现 出现一张就返回 或运算
   def watchOrImgs(self, imgPaths, duration ,intervalTime, threshold = 0.7):
