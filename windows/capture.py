...
from windows.window_api import get_window_rect


def capture(hWnd: int) -> ndarray:
...
def continuous_screen(hWnd: int, pictures: int, img_path: str, screen_sleep_seconds: float = 0.1,
...
def template_matching(hWnd: int, img_path: str) -> tuple:
...
def target_identification(hWnd: int, img_path: str) -> tuple[int, int]:
    """ 模板图片 截图识别位置 返回中心点坐标 """
    img, template_img, result = template_matching(hWnd, img_path)
    # 获取最大值和最小值以及他们的坐标
...
def target_identification_all(hWnd: int, img_path: str, accuracy: float) -> list[tuple[int, int]]:
...
__all__ = ['capture', 'template_matching', 'target_identification',
